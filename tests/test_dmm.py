...
@pytest.mark.parametrize(
    'kwargs',
    [{'function': 'volt:dc', 'range': 10, 'nplc': 10, 'auto_zero': 'off'},
     {'function': 'dcv', 'range': 10.0, 'nplc': 10.0, 'auto_zero': Auto.OFF},
     ])
def test_create_scanner_channel_1(kwargs):
    channel = Keithley6500.create_scanner_channel(1, **kwargs)
    assert channel.number == 1
    assert channel.function is Function.DCV
    assert channel.range == 10.0
    assert channel.nplc == 10.0
    assert channel.auto_zero is Auto.OFF
...
def test_create_scanner_channel_range(rng, expected):
    channel = Keithley6500.create_scanner_channel(2, range=rng)
    assert channel.number == 2
    assert channel.function is Function.DCV
    assert channel.range == expected
    assert channel.nplc == 10.0
    assert channel.auto_zero is Auto.ON
...
def test_create_scanner_channel_raises(kwargs):
    with pytest.raises(ValueError):
        Keithley6500.create_scanner_channel(1, **kwargs)
