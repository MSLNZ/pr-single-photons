...
def test_kwargs():
    ...
    with pytest.raises(TypeError, match='1 to 2 positional arguments'):
        Samples(np.arange(10), 1.0, std=1.0)  # noqa
    with pytest.raises(TypeError, match='1 to 2 positional arguments'):
...
def test_mean_std(recwarn):
    ...
    assert math.isinf(s.stdom)
    assert math.isnan(s.relative_std)
    assert math.isinf(s.relative_stdom)
    assert math.isnan(s.variance)
    ...
    assert s.std == 1.0
    assert math.isinf(s.stdom)
    assert math.isinf(s.relative_std)
    assert s.variance == 1.0

    s = Samples(mean=1.23, std=1.23, size=10)
    assert s.samples.size == 0
    assert s.size == 10
    assert s.mean == 1.23
    assert s.std == 1.23
    assert s.stdom == pytest.approx(0.3889601522007106)
    assert s.relative_std == 100.
...
def test_nan_inf():
    ...
    s = Samples(mean=np.inf, std=np.nan, size=1)
    assert f'{s}' == 'inf(nan)'
    ...
    assert f'{s}' == '3.14(inf)'
    assert f'{s:B}' == '3.14(inf)'
    assert f'{s:P}' == '3.14+/-inf'
    assert f'{s:f}' == '3.14(inf)'
    assert f'{s:e}' == '3.14(inf)e+00'
    assert f'{s:g}' == '3.1(inf)'
    assert f'{s:F}' == '3.14(INF)'
    assert f'{s:E}' == '3.14(INF)E+00'
    assert f'{s:G}' == '3.1(INF)'
    assert f'{s:fP}' == '3.14+/-inf'
    assert f'{s:eP}' == '(3.14+/-inf)e+00'
    assert f'{s:gP}' == '3.1+/-inf'
    assert f'{s:FP}' == '3.14+/-INF'
    assert f'{s:EP}' == '(3.14+/-INF)E+00'
    assert f'{s:GP}' == '3.1+/-INF'
    assert f'{s:.4f}' == '3.1416(inf)'
    assert f'{s:.4e}' == '3.1416(inf)e+00'
    assert f'{s:.4g}' == '3.142(inf)'
    assert f'{s:.4F}' == '3.1416(INF)'
    assert f'{s:.4E}' == '3.1416(INF)E+00'
    assert f'{s:.4G}' == '3.142(INF)'
    assert f'{s:.4fP}' == '3.1416+/-inf'
    assert f'{s:.4eP}' == '(3.1416+/-inf)e+00'
    assert f'{s:.4gP}' == '3.142+/-inf'
    assert f'{s:.4FP}' == '3.1416+/-INF'
    assert f'{s:.4EP}' == '(3.1416+/-INF)E+00'
    assert f'{s:.4GP}' == '3.142+/-INF'
    assert f'{s:.4}' == '3.1416(inf)'
    assert f'{s:.4B}' == '3.1416(inf)'
    assert f'{s:.4P}' == '3.1416+/-inf'
    ...
    assert f'{s:.3S}' == '187(inf) M'
    assert f'{s:.3PS}' == '187+/-inf M'

    s = Samples(mean=1.8667540e4)
    assert f'{s:S}' == '19(inf) k'
    assert f'{s:.6PS}' == '18.6675+/-inf k'

    s = Samples(mean=1.8667540e-6)
    assert f'{s:.1US}' == '2(inf) µ'
    assert f'{s: .2PS}' == ' 1.9+/-inf u'
    assert f'{s:.5PUS}' == '1.8668±inf µ'


def test_bracket_type_f():
    s = Samples(mean=1.23456789, std=0.0123456789, size=1)
    assert f'{s:.1}' == '1.23(1)'
    ...
    factor = 10 ** -20
    s = Samples(mean=1.23456789 * factor, std=0.0123456789 * factor, size=1)
    ...
    s = Samples(mean=1.23456789, std=1234.56789, size=1)
    assert f'{s: .2}' == ' 0(1200)'

    s = Samples(mean=1.23456789, std=123.456789, size=1)
    assert f'{s:.2}' == '0(120)'

    s = Samples(mean=1.23456789, std=12.3456789, size=1)
    assert f'{s: }' == ' 1(12)'

    s = Samples(mean=1.23456789, std=1.23456789, size=1)
    assert f'{s:}' == '1.2(1.2)'

    s = Samples(mean=1.23456789, std=0.123456789, size=1)
    assert f'{s}' == '1.23(12)'

    s = Samples(mean=1.23456789, std=0.0123456789, size=1)
    assert f'{s}' == '1.235(12)'

    s = Samples(mean=1.23456789, std=0.00123456789, size=1)
    assert f'{s}' == '1.2346(12)'

    s = Samples(mean=1.23456789, std=0.000123456789, size=1)
    assert f'{s}' == '1.23457(12)'

    s = Samples(mean=1.23456789, std=0.000000123456789, size=1)
    assert f'{s}' == '1.23456789(12)'

    s = Samples(mean=1.23456789, std=0.000000000123456789, size=1)
    assert f'{s}' == '1.23456789000(12)'

    s = Samples(mean=1.23456789e-4, std=0.000000000123456789, size=1)
    assert f'{s}' == '0.00012345679(12)'

    s = Samples(mean=1.23456789e4, std=0.000000123456789, size=1)
    ...
    s = Samples(mean=123456789., std=1234.56789, size=1)
    assert f'{s:.6e}' == '1.2345678900(123457)e+08'
    assert f'{s:.1f}' == '123457000(1000)'

    s = Samples(mean=1.23456789, std=0.12345, size=1)
    assert f'{s:.1}' == '1.2(1)'
    assert f'{s:.4}' == '1.2346(1235)'

    s = Samples(mean=1.23456789, std=0.945, size=1)
    assert f'{s:.1f}' == '1.2(9)'

    s = Samples(mean=-1.23456789, std=0.945, size=1)
    assert f'{s:.2f}' == '-1.23(94)'

    s = Samples(mean=1.23456789, std=0.95, size=1)
    assert f'{s:.1}' == '1.2(9)'
    assert f'{s:+.3f}' == '+1.235(950)'

    s = Samples(mean=1.23456789, std=0.951, size=1)
    assert f'{s:.1f}' == '1(1)'
    assert f'{s:.2f}' == '1.23(95)'
    assert f'{s:.3f}' == '1.235(951)'

    s = Samples(mean=1.23456789, std=0.999999999999, size=1)
    assert f'{s:.1}' == '1(1)'
    assert f'{s:.2}' == '1.2(1.0)'
    assert f'{s:.5}' == '1.2346(1.0000)'

    s = Samples(mean=1.23456789, std=1.5, size=1)
    assert f'{s:.1}' == '1(2)'

    s = Samples(mean=1.23456789, std=9.5, size=1)
    assert f'{s:.1f}' == '0(10)'

    s = Samples(mean=1.23456789, std=10.00, size=1)
    assert f'{s:.1f}' == '0(10)'

    s = Samples(mean=123.456789, std=0.321, size=1)
    assert f'{s:.1f}' == '123.5(3)'
    assert f'{s}' == '123.46(32)'

    s = Samples(mean=123.456789, std=0.95, size=1)
    assert f'{s:.1}' == '123.5(9)'
    assert f'{s:.3f}' == '123.457(950)'

    s = Samples(mean=123.456789, std=0.951, size=1)
    assert f'{s:.1}' == '123(1)'
    assert f'{s:.4}' == '123.4568(9510)'

    s = Samples(mean=123.456789, std=0.999999999999999, size=1)
    assert f'{s:.1f}' == '123(1)'

    s = Samples(mean=-123.456789, std=0.999999999999999, size=1)
    assert f'{s:.6}' == '-123.45679(1.00000)'

    s = Samples(mean=0.9876, std=0.1234, size=1)
    assert f'{s:.1f}' == '1.0(1)'
    assert f'{s:.3f}' == '0.988(123)'

    s = Samples(mean=0.000003512, std=0.00000006551, size=1)
    assert f'{s:.1}' == '0.00000351(7)'
    assert f'{s}' == '0.000003512(66)'

    s = Samples(mean=0.000003512, std=0.0000008177, size=1)
    assert f'{s:.1f}' == '0.0000035(8)'
    assert f'{s:.3}' == '0.000003512(818)'

    s = Samples(mean=0.000003512, std=0.000009773, size=1)
    assert f'{s:.1}' == '0.00000(1)'
    assert f'{s:.4}' == '0.000003512(9773)'

    s = Samples(mean=0.000003512, std=0.00001241, size=1)
    assert f'{s:.1}' == '0.00000(1)'
    assert f'{s}' == '0.000004(12)'

    s = Samples(mean=0.000003512, std=0.0009998, size=1)
    assert f'{s:.1}' == '0.000(1)'
    assert f'{s:.4f}' == '0.0000035(9998)'

    s = Samples(mean=0.000003512, std=0.006563, size=1)
    assert f'{s:.1f}' == '0.000(7)'
    assert f'{s:}' == '0.0000(66)'

    s = Samples(mean=0.000003512, std=0.09564, size=1)
    assert f'{s:.1}' == '0.0(1)'
    assert f'{s:.4f}' == '0.00000(9564)'

    s = Samples(mean=0.000003512, std=0.7772, size=1)
    assert f'{s:.1}' == '0.0(8)'

    s = Samples(mean=0.000003512, std=9.75, size=1)
    assert f'{s:.1}' == '0(10)'

    s = Samples(mean=0.000003512, std=33.97, size=1)
    assert f'{s:.1}' == '0(30)'

    s = Samples(mean=0.000003512, std=715.5, size=1)
    assert f'{s:.1}' == '0(700)'
    assert f'{s:.5f}' == '0.00(715.50)'

    s = Samples(mean=0.07567, std=0.00000007018, size=1)
    assert f'{s:.1f}' == '0.07567000(7)'
    assert f'{s:.5}' == '0.075670000000(70180)'

    s = Samples(mean=0.07567, std=0.0000003645, size=1)
    assert f'{s:.1}' == '0.0756700(4)'

    s = Samples(mean=-0.07567, std=0.0000003645, size=1)
    assert f'{s:.3f}' == '-0.075670000(365)'

    s = Samples(mean=0.07567, std=0.000005527, size=1)
    assert f'{s:.1}' == '0.075670(6)'
    assert f'{s: .2F}' == ' 0.0756700(55)'

    s = Samples(mean=0.07567, std=0.00004429, size=1)
    assert f'{s:.1f}' == '0.07567(4)'
    assert f'{s}' == '0.075670(44)'

    s = Samples(mean=0.07567, std=0.0008017, size=1)
    assert f'{s:.1}' == '0.0757(8)'
    assert f'{s:.3}' == '0.075670(802)'

    s = Samples(mean=0.07567, std=0.006854, size=1)
    assert f'{s:.1}' == '0.076(7)'
    assert f'{s:.4}' == '0.075670(6854)'

    s = Samples(mean=0.07567, std=0.06982, size=1)
    assert f'{s:.1}' == '0.08(7)'
    assert f'{s}' == '0.076(70)'

    s = Samples(mean=0.07567, std=0.7382, size=1)
    assert f'{s:.1}' == '0.1(7)'
    assert f'{s:.3}' == '0.076(738)'

    s = Samples(mean=0.07567, std=7.436, size=1)
    assert f'{s:.1}' == '0(7)'
    assert f'{s}' == '0.1(7.4)'

    s = Samples(mean=0.07567, std=48.75, size=1)
    assert f'{s:.1}' == '0(50)'
    assert f'{s:.3}' == '0.1(48.8)'

    s = Samples(mean=0.07567, std=487.9, size=1)
    assert f'{s:.1}' == '0(500)'
    assert f'{s:.5f}' == '0.08(487.90)'

    s = Samples(mean=8.545, std=0.00000007513, size=1)
    assert f'{s:.1}' == '8.54500000(8)'
    assert f'{s}' == '8.545000000(75)'

    s = Samples(mean=8.545, std=0.000009935, size=1)
    assert f'{s:.1}' == '8.54500(1)'
    assert f'{s:.2}' == '8.5450000(99)'

    s = Samples(mean=8.545, std=0.003243, size=1)
    assert f'{s:.1}' == '8.545(3)'
    assert f'{s:.3}' == '8.54500(324)'

    s = Samples(mean=8.545, std=0.0812, size=1)
    assert f'{s:.1}' == '8.54(8)'
    assert f'{s}' == '8.545(81)'

    s = Samples(mean=8.545, std=0.4293, size=1)
    assert f'{s:.1}' == '8.5(4)'
    assert f'{s:.4}' == '8.5450(4293)'

    s = Samples(mean=8.545, std=6.177, size=1)
    assert f'{s:.1}' == '9(6)'
...
def test_bracket_type_e_ureal():
    ...
    s = Samples(mean=123.456789, std=1.0, size=1)
    assert f'{s:.1e}' == '1.23(1)e+02'
    assert f'{s:E}' == '1.235(10)E+02'

    s = Samples(mean=123.456789, std=9.123, size=1)
    assert f'{s:.1e}' == '1.23(9)e+02'
    assert f'{s:.4e}' == '1.23457(9123)e+02'

    s = Samples(mean=123.456789, std=9.9, size=1)
    assert f'{s:.1e}' == '1.2(1)e+02'
    assert f'{s:e}' == '1.235(99)e+02'

    s = Samples(mean=123.456789, std=94.9, size=1)
...
def test_type_g():
    ...
    s = Samples(mean=7.2524e-8, std=5.429e-10, size=1)
    assert f'{s:.2g}' == '7.252(54)e-08'
    assert f'{s:.1G}' == '7.25(5)E-08'

    s = Samples(mean=7.2524e4, std=5.429e3, size=1)
...
def test_unicode():
    s = Samples(mean=18.5424, std=0.94271, size=1)
    ...
    factor = 1e-6
    s = Samples(mean=1.23456789 * factor, std=0.123456789 * factor, size=1)
    ...
    s = Samples(mean=1.23456789 * factor, std=0.123456789 * factor, size=1, overload=math.inf)
    assert f'{s:.3eU}' == '1.235(123)×10¹⁰⁰'
...
def test_hash_symbol():
    s = Samples(mean=5.4, std=1.2, size=1)
    assert f'{s:#.1}' == '5.(1.)'
    assert f'{s:#}' == '5.4(1.2)'

    s = Samples(mean=1, std=0.001, size=1)
    assert f'{s:#.1}' == '1.000(1)'

    s = Samples(mean=1, std=0.1, size=1)
    assert f'{s:#.1}' == '1.0(1)'

    s = Samples(mean=1, std=1, size=1)
    assert f'{s:.1}' == '1(1)'
    assert f'{s:#.1}' == '1.(1.)'

    s = Samples(mean=1, std=0.9876, size=1)
    assert f'{s:#.1}' == '1.(1.)'

    s = Samples(mean=1, std=0.9876, size=1)
    assert f'{s:#.2f}' == '1.00(99)'

    s = Samples(mean=1, std=10, size=1)
    assert f'{s:#.1}' == '0.(10.)'

    s = Samples(mean=1, std=1000, size=1)
    assert f'{s:#.1}' == '0.(1000.)'

    s = Samples(mean=12345, std=9876, size=1)
    assert f'{s:#e}' == '1.23(99)e+04'
    assert f'{s:#}' == '12300.(9900.)'

    s = Samples(mean=10, std=10, size=1)
    assert f'{s:#.1E}' == '1.(1.)E+01'


def test_grouping_field():
    s = Samples(mean=123456789, std=123456, size=1)
...
def test_zero_field():
    s = Samples(mean=1.342, std=0.0041, size=1)
...
def test_latex():
    ...
    s = Samples(mean=3.14159)
    assert f'{s:fL}' == r'3.14\left(\infty\right)'
    assert f'{s:.4fL}' == r'3.1416\left(\infty\right)'

    s = Samples(mean=np.nan, std=3.142, size=1)
    assert f'{s:L}' == r'\mathrm{NaN}\left(3.142\right)'

    s = Samples(mean=-np.inf, std=np.inf, size=1)
    assert f'{s:FL}' == r'-\infty\left(\infty\right)'


def test_percent_type():
    s = Samples(mean=0.1548175123, std=0.0123456, size=1)
    assert f'{s:.1%}' == '15(1)%'
    assert f'{s:.4%}' == '15.482(1.235)%'
    assert f'{s:.3%L}' == r'15.48\left(1.23\right)\%'

    s = Samples(mean=0.1548175123, std=0.000123456, size=1)
    assert f'{s:%L}' == r'15.482\left(12\right)\%'
    assert f'{s:%U}' == '15.482(12)%'


def test_plus_minus():
    s = Samples(mean=1.0, std=0.000123456789, size=1)
    assert f'{s:+.4fP}' == '+1.0000000+/-0.0001235'

    s = Samples(mean=7.2524, std=0.0032153, size=1)
    assert f'{s:.4fP}' == '7.252400+/-0.003215'

    s = Samples(mean=-1.2345, std=123.456789, size=1)
    assert f'{s:12.5fP}' == '-1.23+/-123.46'

    s = Samples(mean=1.5431384e-8, std=4.32856e-12, size=1)
    assert f'{s:P}' == '0.0000000154314+/-0.0000000000043'
    assert f'{s:eP}' == '(1.54314+/-0.00043)e-08'

    s = Samples(mean=1.5431384e7, std=4.32856e6, size=1)
...
def test_si():
    s = Samples(mean=2.37216512e-32, std=1.3721356e-32, size=1)
    assert f'{s:.1S}' == '0.02(1) q'
    assert f'{s:.3PS}' == '(0.0237+/-0.0137) q'

    s = Samples(mean=4.638174e-30, std=0.0635119e-30, size=1)
    assert f'{s:.2S}' == '4.638(64) q'
    assert f'{s:.1PS}' == '(4.64+/-0.06) q'

    s = Samples(mean=8.2621518e-29, std=0.00826725e-29, size=1)
    assert f'{s:.3S}' == '82.6215(827) q'
    assert f'{s:.1PS}' == '(82.62+/-0.08) q'

    s = Samples(mean=6.547251e-27, std=0.07541268e-27, size=1)
    assert f'{s:.1S}' == '6.55(8) r'
    assert f'{s:PS}' == '(6.547+/-0.075) r'

    s = Samples(mean=9.092349e-25, std=0.0038964e-25, size=1)
    assert f'{s:.3S}' == '909.235(390) r'
    assert f'{s:PS}' == '(909.23+/-0.39) r'

    s = Samples(mean=5.206637324e-24, std=0.415002e-24, size=1)
    assert f'{s:.1S}' == '5.2(4) y'
    assert f'{s:.5PS}' == '(5.20664+/-0.41500) y'

    s = Samples(mean=9.6490243e-22, std=0.058476272e-22, size=1)
    assert f'{s:.4S}' == '964.902(5.848) y'
    assert f'{s:.1PS}' == '(965+/-6) y'

    s = Samples(mean=6.2860846e-20, std=0.02709243e-20, size=1)
    assert f'{s:S}' == '62.86(27) z'
    assert f'{s:PS}' == '(62.86+/-0.27) z'

    s = Samples(mean=5.2032008e-17, std=0.00084681469e-17, size=1)
    assert f'{s:S}' == '52.0320(85) a'
    assert f'{s:PS}' == '(52.0320+/-0.0085) a'

    s = Samples(mean=8.541971e-15, std=1.93486e-15, size=1)
    assert f'{s:.3S}' == '8.54(1.93) f'
    assert f'{s:.1PS}' == '(9+/-2) f'

    s = Samples(mean=8.541971e-14, std=1.93486e-14, size=1)
    assert f'{s:.3S}' == '85.4(19.3) f'
    assert f'{s:.1PS}' == '(90+/-20) f'

    s = Samples(mean=8.125524e-10, std=0.043966e-10, size=1)
    assert f'{s:.1S}' == '813(4) p'
    assert f'{s:.4PS}' == '(812.552+/-4.397) p'

    s = Samples(mean=1.7540272e-9, std=6.5160764e-9, size=1)
    assert f'{s:.3S}' == '1.75(6.52) n'
    assert f'{s:.3FS}' == '1.75(6.52) n'
    assert f'{s:.4PS}' == '(1.754+/-6.516) n'

    s = Samples(mean=4.5569880e-7, std=0.004160764e-7, size=1)
    assert f'{s:.1S}' == '455.7(4) n'
    assert f'{s:PS}' == '(455.70+/-0.42) n'

    s = Samples(mean=9.2863e-4, std=0.70230056610e-4, size=1)
    ...
    s = Samples(mean=5.6996491e-2, std=0.5302890e-2, size=1)
    assert f'{s:.4S}' == '56.996(5.303) m'
    assert f'{s:.1PS}' == '(57+/-5) m'

    s = Samples(mean=2.69364683, std=0.00236666, size=1)
    assert f'{s:BUS}' == '2.6936(24)'
    assert f'{s:.3PS}' == '2.69365+/-0.00237'

    s = Samples(mean=4.4733994e2, std=0.1085692e2, size=1)
    assert f'{s:.1S}' == '450(10)'
    assert f'{s:.4PS}' == '447.34+/-10.86'

    s = Samples(mean=8.50987467e4, std=0.6095151e4, size=1)
    assert f'{s:S}' == '85.1(6.1) k'
    assert f'{s:.1PS}' == '(85+/-6) k'

    s = Samples(mean=1.8e6, std=0.0453589e6, size=1)
    assert f'{s:.4S}' == '1.80000(4536) M'
    assert f'{s:.3PUS}' == '(1.8000±0.0454) M'

    s = Samples(mean=1.8667540e8, std=0.00771431e8, size=1)
    assert f'{s:.3S}' == '186.675(771) M'
    assert f'{s:.3PS}' == '(186.675+/-0.771) M'

    s = Samples(mean=7.789499e9, std=0.7852736e9, size=1)
    assert f'{s:.1S}' == '7.8(8) G'
    assert f'{s:PS}' == '(7.79+/-0.79) G'

    s = Samples(mean=2.2038646e13, std=12.743090e13, size=1)
    assert f'{s:.1S}' == '0(100) T'
    assert f'{s:.2fPS}' == '(20+/-130) T'

    s = Samples(mean=6.084734e16, std=1.2485885e16, size=1)
    assert f'{s:.3S}' == '60.8(12.5) P'
    assert f'{s:PS}' == '(61+/-12) P'

    s = Samples(mean=7.66790e18, std=0.05647e18, size=1)
    assert f'{s:.4S}' == '7.66790(5647) E'
    assert f'{s:.6PS}' == '(7.6679000+/-0.0564700) E'

    s = Samples(mean=3.273545e22, std=0.004964854e22, size=1)
    assert f'{s:.1S}' == '32.74(5) Z'
    assert f'{s:.1PS}' == '(32.74+/-0.05) Z'

    s = Samples(mean=4.32184e24, std=0.0005879417e24, size=1)
    assert f'{s:.3S}' == '4.321840(588) Y'
    assert f'{s:.2PS}' == '(4.32184+/-0.00059) Y'

    s = Samples(mean=752.987235265e24, std=0.32198187e24, size=1)
    assert f'{s:.1S}' == '753.0(3) Y'
    assert f'{s:.3PS}' == '(752.987+/-0.322) Y'

    s = Samples(mean=1.638324e27, std=0.773148e27, size=1)
    assert f'{s:S}' == '1.64(77) R'
    assert f'{s:.4PS}' == '(1.6383+/-0.7731) R'

    s = Samples(mean=9.276154e28, std=0.02473e28, size=1)
    assert f'{s:.2S}' == '92.76(25) R'
    assert f'{s:.2PS}' == '(92.76+/-0.25) R'

    s = Samples(mean=0.876236e30, std=0.009236721e30, size=1)
    assert f'{s:.1S}' == '876(9) R'
    assert f'{s:.2PS}' == '(876.2+/-9.2) R'

    s = Samples(mean=4.32687514e31, std=0.9211314e31, size=1, overload=1e99)
    assert f'{s:.3S}' == '43.27(9.21) Q'
    assert f'{s:.1PS}' == '(43+/-9) Q'

    s = Samples(mean=7.582761830927e34, std=0.027125e34, size=1, overload=1e99)
    assert f'{s:.1S}' == '75800(300) Q'
    assert f'{s:.4PS}' == '(75827.6+/-271.2) Q'


def test_iterable():
    s = Samples(mean=1.2, std=0.32, size=1)
    mean, stdom = s
    assert mean == 1.2
    assert stdom == 0.32
    assert 'mean={}, stdom={}'.format(*s) == 'mean=1.2, stdom=0.32'

    s = Samples(mean=1.2, std=0.32, size=100)
    mean, stdom = s
    assert mean == 1.2
    assert stdom == 0.032
    assert 'mean={}, stdom={}'.format(*s) == 'mean=1.2, stdom=0.032'
...
def test_type_n_swiss():
    ...
    s = Samples(mean=1.23456789, std=0.987654321, size=1)
    if sys.platform == 'darwin':
        assert f'{s:n}' == '1,23(99)'
    else:
        assert f'{s:n}' == '1.23(99)'

    s = Samples(mean=1.2345678987e6, std=0.987654321, size=1)
    if sys.platform == 'darwin':
        assert f'{s:.4n}' == '1234567,8987(9877)'
    else:
        assert f'{s:.4n}' == '1’234’567.8987(9877)'

    s = Samples(mean=12345.6789, std=9876.54321, size=1)
    if sys.platform == 'darwin':
...
def test_type_n_german():
    ...
    s = Samples(mean=2345, std=1234, size=1)
...
def test_to_ureal():
    ...
    s = Samples(data)
    un = s.to_ureal()
    un2 = type_a.estimate(data)
    assert un.x == pytest.approx(126652.5)
    assert un.x == pytest.approx(un2.x)
    assert un.u == pytest.approx(227.19325763716992)
    assert un.u == pytest.approx(un2.u)
    assert un.df == 19
    assert un.df == un2.df
    assert un.label is None
